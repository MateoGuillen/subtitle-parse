...
def match_titles_with_lines(
    outlines_path: str = "merged_outlines.parquet",
    pdf_lines_path: str = "combined_documents_all_years.parquet",
    output_path: str = "./merged_outlines_with_lines.parquet",
):
    """
    ...
    try:
        ...
        logging.info(
            f"Loaded {len(outlines_df)} outline entries and {len(pdf_lines_df)} PDF lines"
        )
