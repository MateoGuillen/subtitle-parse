...
def load_dataframes(
    pdf_lines_path: str, outlines_path: str
...
def preprocess_dataframes(
    pdf_lines_df: pd.DataFrame, outlines_df: pd.DataFrame
...
def extract_content_sections(
    pdf_lines_dict: Dict, outlines_df: pd.DataFrame
) -> List[ContentSection]:
    """
...
def main():
    ...
    try:
        ...
        logging.info(
            f"Loaded {len(outlines_df)} outlines and {len(pdf_lines_df)} PDF lines"
        )
